import { AccsCOSMOSParams, AccsDefaultParams, AccsEVMParams, AccsRegularParams, AccsSOLV2Params, ALL_LIT_CHAINS, CheckAndSignAuthParams, DecryptZipFileWithMetadata, DecryptZipFileWithMetadataProps, EncryptedFile, EncryptedString, EncryptedZip, EncryptFileAndZipWithMetadataProps, ILitError, JsonAuthSig, LIT_ERROR_TYPE, ThreeKeys, VMTYPE } from "@litprotocol-dev/constants";
import { checkIfAuthSigRequiresChainParam, checkType, log, throwError, safeParams } from "@litprotocol-dev/utils";
 ...
 */
 export const decryptFile = ({ file, symmetricKey }) => {
  // -- validate
  if (
    !checkType({
      value: file,
