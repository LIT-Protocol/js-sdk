 ...
 * @returns { JsonAuthSig } 
 */
export const checkAndSignSolAuthMessage = async () : Promise<JsonAuthSig | void> => {
 ...
 * 
 */
export const signAndSaveAuthMessage = async ({ provider } : {
    provider: any
    ...
    localStorage.setItem(LOCAL_STORAGE_KEYS.AUTH_SOL_SIGNATURE, JSON.stringify(authSig));
