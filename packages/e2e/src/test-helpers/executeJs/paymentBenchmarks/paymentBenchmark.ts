...
export const registerPaymentBenchmarkTests = () => {
  ...
  beforeAll(async () => {
    ...
    if (!benchmarkUser.eoaAuthContext) {
      throw new Error(
        'EOA auth context was not created for payment benchmark test'
      );
    }
  ...
  describe('Payment Benchmark Tests', () => {
    describe('Secure API Key Usage', () => {
      test.skip('should encrypt outside the Lit Action, and decrypt and make a fetch request inside the Lit Action', async () => {
      ...
      }, 120000); // 2 minute timeout for the test (includes 5s wait in action)
