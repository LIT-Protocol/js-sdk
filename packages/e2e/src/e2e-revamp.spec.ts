 ...
 *
 * - Just this file:
 *   `NETWORK=naga-dev pnpm exec dotenvx run --env-file=.env -- jest --runInBand --config ./jest.e2e.config.ts --cacheDirectory .jest-cache --runTestsByPath packages/e2e/src/e2e-revamp.spec.ts`
 *
 * - Single test / suite (use Jest name filtering):
 *   `NETWORK=naga-dev pnpm exec dotenvx run --env-file=.env -- jest --runInBand --config ./jest.e2e.config.ts --cacheDirectory .jest-cache --runTestsByPath packages/e2e/src/e2e-revamp.spec.ts --testNamePattern "EOA auth.*pkpSign"`
 *
...
describe('revamped e2e suite', () => {
  ...
  beforeAll(async () => {
    ...
    resolvedNetwork = { name, importName, type, networkModule: testEnv.networkModule };
