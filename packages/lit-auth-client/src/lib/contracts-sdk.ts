...
export class LitContracts {
   ...
   *
   * @param {any} [args] An optional value to logDebug with the message.
   */
  logDebug = (...args: any) => {
    if (this.debug) {
  ...
  connect = async () => {
    ...
    if (isBrowser() && !this.signer) {
      this.logDebug("----- We're in the browser! -----");
    ...
    // ======================================
    if (this.privateKey) {
      this.logDebug('Using your own private key');
    ...
    if (
      ...
      try {
        ...
        // swallow
        // this.logDebug('Not a problem.');
      }

      // -- (NOT FOUND) no private key found
      if (!storagePrivateKey) {
        this.logDebug('Not a problem, we will generate a random private key');
        storagePrivateKey = ethers.utils.hexlify(ethers.utils.randomBytes(32));
      }

      // -- (FOUND) private key found
      else {
        this.logDebug("Found your private key in local storage. Let's use it!");
      }

      this.signer = new ethers.Wallet(storagePrivateKey, this.provider);

      this.logDebug('- Your private key:', storagePrivateKey);
      this.logDebug('- Your address:', await this.signer.getAddress());
      this.logDebug('- this.signer:', this.signer);
      this.logDebug('- this.provider.getSigner():', this.provider.getSigner());
  ...
  // ***********************************************************************************************
    ...
    this.logDebug('Your Signer:', this.signer);
    this.logDebug('Your Provider:', this.provider);

    if (!this.provider) {
      this.logDebug(
        'No provide found. Will try to use the one from the signer.'
      );
      this.provider = this.signer.provider;
      this.logDebug('Your Provider(from signer):', this.provider);
    }

    let addresses: any = await LitContracts.getContractAddresses(this.network);
    this.logDebug('resolved contract addresses for: ', this.network, addresses);
    // ----- autogen:init:start  -----
    // Generated at 2023-11-07T01:50:52.460Z
...
https://developer.litprotocol.com/v3/sdk/wallets/auth-methods/#auth-method-scopes
      ...
      getTokensByAddress: async (
        ...
        for (let i = 0; ; i++) {
          ...
          try {
            ...
            this.logDebug(`[getTokensByAddress] Ended search on index: ${i}`);
            break;
          }
        }
    ...
    write: {
      mint: async () => {
        ...
        if (this.isPKP) {
          ...
          this.logDebug('...signing tx');
          let signedTx = await this.signer.signTransaction(tx);
          this.logDebug('signedTx:', signedTx);

          this.logDebug('sending signed tx...');
  ...
  pkpPermissionsContractUtils = {
    read: {
      ...
      getPermittedAddresses: async (
        ...
        this.logDebug('[getPermittedAddresses] input<tokenId>:', tokenId);
      ...
      isPermittedAction: async (
        ...
        this.logDebug('[isPermittedAction] input<pkpId>:', pkpId);
        this.logDebug('[isPermittedAction] input<ipfsId>:', ipfsId);

        const ipfsHash = this.utils.getBytesFromMultihash(ipfsId);
        this.logDebug('[isPermittedAction] converted<ipfsHash>:', ipfsHash);
    ...
    write: {
      ...
      addPermittedAction: async (
        ...
        this.logDebug('[addPermittedAction] input<pkpId>:', pkpId);

        const pubKey = await this.pubkeyRouterContract.read.getPubkey(pkpId);
        this.logDebug('[addPermittedAction] converted<pubKey>:', pubKey);

        const pubKeyHash = ethers.utils.keccak256(pubKey);
        this.logDebug(
          '[addPermittedAction] converted<pubKeyHash>:',
          pubKeyHash
        );

        const tokenId = ethers.BigNumber.from(pubKeyHash);
        this.logDebug('[addPermittedAction] converted<tokenId>:', tokenId);

        this.logDebug('[addPermittedAction] input<ipfsId>:', ipfsId);
        ...
        this.logDebug('[addPermittedAction] output<tx>:', tx);
      ...
      addPermittedAddress: async (
        ...
        this.logDebug('[addPermittedAddress] input<pkpId>:', pkpId);
        this.logDebug(
          '[addPermittedAddress] input<ownerAddress>:',
          ownerAddress
        );
        ...
        this.logDebug('[addPermittedAddress] output<tx>:', tx);
      ...
      revokePermittedAction: async (
        ...
        this.logDebug('[revokePermittedAction] input<pkpId>:', pkpId);
        this.logDebug('[revokePermittedAction] input<ipfsId>:', ipfsId);

        const ipfsHash = this.utils.getBytesFromMultihash(ipfsId);
        this.logDebug('[revokePermittedAction] converted<ipfsHash>:', ipfsHash);
        ...
        this.logDebug('[revokePermittedAction] output<tx>:', tx);
  ...
  routerContractUtils = {
    read: {
      ...
      //   this.logDebug('[getIpfsIds] input<solidityIpfsId>:', solidityIpfsId);
      //   const ipfsId = this.utils.getMultihashFromBytes(solidityIpfsId);
      //   this.logDebug('[getIpfsIds] output<ipfsId>:', ipfsId);
      //   return ipfsId;
      // },
    },
    write: {},
