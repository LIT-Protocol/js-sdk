...
export const handleMultiAuths = async (
  authDataArray: LitAuthMethod[]
): Promise<PKPInfo> => {
  log.start('handleMultiAuths', 'handle-multi-auths.ts');

  log.info('authDataArray:', authDataArray);

  const authMethodTypes = authDataArray.map(
    (authData) => authData.authMethodType
  );

  const authMethodIds = await Promise.all(
    authDataArray.map(async (authData) => {
  ...
  const authMethodPubKeys = authDataArray.map((auth) => {
  ...
  const scopes = authDataArray.map((_) => [ethers.BigNumber.from('0')]);
