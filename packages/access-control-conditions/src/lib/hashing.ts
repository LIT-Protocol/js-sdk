...
export const hashUnifiedAccessControlConditions = (
  ...
  log(LogLevel.INFO, 'unifiedAccessControlConditions:', unifiedAccessControlConditions);
  ...
  log(LogLevel.DEBUG, 'conditions:', conditions);
  ...
  log(LogLevel.INFO, 'Hashing unified access control conditions: ', toHash);
...
export const hashAccessControlConditions = (
  ...
  log(LogLevel.INFO, 'Hashing access control conditions: ', toHash);
  const encoder = new TextEncoder();
  const data = encoder.encode(toHash);
...
export const hashEVMContractConditions = (
  ...
  const toHash = JSON.stringify(conds);
  log(LogLevel.INFO, 'Hashing evm contract conditions: ', toHash);
  const encoder = new TextEncoder();
  const data = encoder.encode(toHash);
  return crypto.subtle.digest('SHA-256', data);
};
...
export const hashSolRpcConditions = (
  ...
  const toHash = JSON.stringify(conds);
  log(LogLevel.INFO, 'Hashing sol rpc conditions: ', toHash);
  const encoder = new TextEncoder();
  const data = encoder.encode(toHash);
