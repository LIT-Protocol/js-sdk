...
import { createEvmEventState, EventState } from '../../../../shared/StateManager/src/createEvmEventState';
import { createRefreshedValue, RefreshedValue } from '../../../../shared/StateManager/src/createRefreshedValue';
 ...
 */
const areStringArraysDifferent = (
  arr1: string[],
  arr2: string[]
): boolean => {
...
export const createStateManager = async ({
  ...
  const eventStateManager: EventState<STAKING_STATES_VALUES | null> = createEvmEventState<STAKING_STATES_VALUES | null>({
    ...
    onChange: async (newState) => {
      ...
      if (newState === (STAKING_STATES.Active as STAKING_STATES_VALUES)) {
        try {
          ...
          const validatorData = await readOnlyChainManager.api.connection.getConnectionInfo();
          ...
          if (latestEpochInfo?.number !== newEpochInfo.number) {
             _logger.info(`Epoch number updated from ${latestEpochInfo?.number} to ${newEpochInfo.number}`);
             latestEpochInfo = newEpochInfo;
          } else {
             _logger.info(`Epoch number ${newEpochInfo.number} remains the same.`);
          }
  ...
  return {
    ...
    getLatestBlockhash: async (): Promise<string> => {
      try {
        // Assuming getOrRefreshAndGet is the correct method for createRefreshedValue
        const blockhash = await blockhashManager.getOrRefreshAndGet();
        if (!blockhash) {
          throw new Error('Blockhash fetch/refresh returned empty.');
        }
        return blockhash;
      } catch (error) {
    ...
    stop: () => {
      ...
      eventStateManager.stop(); // Assuming eventStateManager has stop()
    },
  };
};
