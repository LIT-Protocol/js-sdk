...
describe('SessionSigs', () => {
  ...
  it.each(sessionGenerators)('DecryptString', async (generator) => {
    ...
    const litActionSessionSigs = await generator.fn(devEnv, alice);
    ...
    const litActionSessionSigs2 = await generator.fn(devEnv, alice, [
      {
  ...
  it.each(sessionGenerators)('Claim Keys', async (generator) => {
    const alice = await devEnv.createRandomPerson();
    const litActionSessionSigs = await generator.fn(devEnv, alice, [
      {
  ...
  it.each(sessionGenerators)('Claim Keys Multiple', async (generator) => {
    devEnv.setUnavailable(LIT_TESTNET.MANZANO);
  ...
  it.each(sessionGenerators)('ExecuteJS JSON Response', async (generator) => {
    devEnv.setUnavailable(LIT_TESTNET.MANZANO);
  ...
  it.each(sessionGenerators)('PKP Sign', async (generator) => {
    devEnv.setUnavailable(LIT_TESTNET.MANZANO);
  ...
  it.each(sessionGenerators)('ExecuteJS Signing', async (generator) => {
    const alice = await devEnv.createRandomPerson();
    const litActionSessionSigs = await generator.fn(devEnv, alice, [
      {
        resource: new LitPKPResource('*'),
  ...
  it.each(sessionGenerators)('ExecteJS Signing Parallel', async (generator) => {
    devEnv.setUnavailable(LIT_TESTNET.MANZANO);
