...
describe('Epoch Transition', () => {
    ...
    it('Transition should trigger staking event', async () => {
        ...
        expect(connectedNodes?.length).toEqual(devEnv.litNodeClient?.config.bootstrapUrls.length);

        
        for (const url of connectedNodes!) {
            expect(devEnv.litNodeClient?.config.bootstrapUrls.indexOf(url)).toBeGreaterThan(-1);
        }
