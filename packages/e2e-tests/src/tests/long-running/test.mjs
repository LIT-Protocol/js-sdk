...
import {
  testEpochTransitionSignSessioShouldValidateAfterTransition,
  testEpochTransitionSignSessioShouldValidateDuringTransition,
  testEpochTransitionHandshakeShouldReturnUniformEpoch,
} from './tests/EpochTransitionSignSession.ts';
...
export const runTestsParallel = async ({ tests, devEnv, chunkSize }) => {
  ...
  const runTest = async ([testName, testFunction], testIndexd) => {
    const maxAttempts = devEnv.processEnvs.MAX_ATTEMPTS;
