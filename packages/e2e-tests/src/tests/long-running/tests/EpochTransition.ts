...
export async function testTransitionEpochShouldTriggerStakingEvent(
  ...
  if (
    connectedNodes?.length !== devEnv.litNodeClient.config.bootstrapUrls.length
  ) {
    throw new Error(
      'Validator collection lengths do not match after epoch transition with same node set'
    );
  }
  ...
  for (const url of connectedNodes!) {
    if (devEnv.litNodeClient.config.bootstrapUrls.indexOf(url) < 0) {
      ...
      throw new Error(
        'Connected nodes does not match original node set pre epoch transiton'
      );
    }
  }
   ...
   */
  if (
    (currentEpoch as number) + 1 !=
    (devEnv.litNodeClient?.currentEpochNumber as number)
  ) {
