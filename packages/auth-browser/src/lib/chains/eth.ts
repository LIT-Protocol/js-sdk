...
export const getChainId = async (
  ...
  try {
    ...
    log(logger, 'getNetwork threw an exception', e);
...
export const getMustResign = (authSig: AuthSig, resources: any): boolean => {
  ...
  try {
    ...
    log(logger, 'parsedSiwe.resources', parsedSiwe.resources);
    ...
    log(logger, 'error parsing siwe sig.  making the user sign again: ', e);
    mustResign = true;
  }
...
export const connectWeb3 = async ({
  ...
  if (isNode()) {
    log(logger, 'connectWeb3 is not supported in nodejs.');
  ...
  log(logger, 'listing accounts');
  const accounts = await web3.listAccounts();

  log(logger, 'accounts', accounts);
...
export const disconnectWeb3 = (): void => {
  if (isNode()) {
    log(logger, 'disconnectWeb3 is not supported in nodejs.');
    return;
  }
...
export const checkAndSignEVMAuthMessage = async ({
  ...
  log(logger, `got web3 and account: ${account}`);
  ...
  log(logger, 'currentChainIdOrError:', currentChainIdOrError);
  log(logger, 'selectedChainId:', selectedChainId);
  log(logger, 'selectedChainIdHex:', selectedChainIdHex);
  log(logger, 'authSigOrError:', authSigOrError);
  ...
  log(logger, 'chainId from web3', currentChainIdOrError);
  log(
    logger,
  ...
  if (currentChainIdOrError.result !== selectedChainId && switchChain) {
    ...
    try {
      log(logger, 'trying to switch to chainId', selectedChainIdHex);
      ...
      log(logger, 'error switching to chainId', switchError);
  ...
  log(logger, 'checking if sig is in local storage');
  ...
  if (authSigOrError.type === EITHER_TYPE.ERROR) {
    ...
    log(logger, '5. authSigOrError:', authSigOrError);
  }
  ...
  if (account.toLowerCase() !== authSig.address.toLowerCase()) {
    ...
    log(logger, '8. mustResign:', mustResign);
  }
  ...
  if (isSignedMessageExpired(authSig.signedMessage) || !checkAuthSig.isValid) {
    if (!checkAuthSig.isValid) {
      log(logger, `Invalid AuthSig: ${checkAuthSig.errors.join(', ')}`);
    }

    log(logger, '9. authSig expired!, resigning..');
...
export const signAndSaveAuthMessage = async ({
  ...
  // check if it's nodejs
  if (isNode()) {
    log(logger, 'checkAndSignEVMAuthMessage is not supported in nodejs.');
    return {
      sig: '',
      derivedVia: '',
      signedMessage: '',
  ...
  log(logger, `generated and saved ${LOCAL_STORAGE_KEYS.KEY_PAIR}`);
  return authSig;
};
...
export const signMessage = async ({
  ...
  // check if it's nodejs
  if (isNode()) {
    log(logger, 'signMessage is not supported in nodejs.');
    return {
      signature: '',
      address: '',
    };
  ...
  // -- validate
  if (!web3 || !account) {
    log(logger, `web3: ${web3} OR ${account} not found. Connecting web3..`);
...
export const signMessageAsync = async (
  ...
  // check if it's nodejs
  if (isNode()) {
    log(logger, 'signMessageAsync is not supported in nodejs.');
    return null;
  }
  ...
  if (signer instanceof JsonRpcSigner) {
    try {
      log(logger, 'Signing with personal_sign');
    ...
    log(logger, 'signing with signMessage');
    return await signer.signMessage(messageBytes);
  }
};
