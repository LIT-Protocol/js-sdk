import { getClient, type ValidateAbilityExecutionAndGetPoliciesResult } from '@lit-protocol/vincent-contracts-sdk';
...
export async function decryptWrappedKeyForDelegatee({
    ...
    await validateDelegateeDelegation({
        appDelegateeAddress,
        agentWalletPkpEthAddress,
        abilityIpfsCid,
    });
...
async function validateDelegateeDelegation({
    appDelegateeAddress,
    agentWalletPkpEthAddress,
    abilityIpfsCid,
}: {
