...
import { AuthResourceTypes } from './auth-types';
...
type ISOString = `${string}-${string}-${string}T${string}:${string}:${string}Z` | string;

export type LitSessionType = 'eoa' | 'pkp' | 'lit-action';
export interface LitSession<T> {
  type: LitSessionType;
  resources: {
    type: AuthResourceTypes,
    request: string | '*';
  }[],
