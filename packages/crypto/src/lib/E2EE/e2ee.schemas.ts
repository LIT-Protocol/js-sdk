 ...
 * }
 */
export const EncryptedPayloadV1Schema = z
  .object({
    V1: z.object({
      verification_key: z
        .string()
      ...
      random: z
        ...
        .regex(
          /^[0-9a-fA-F]{32}$/,
          'Must be 32 character hex string (16 bytes)'
        ),
      created_at: z.string(),
      ciphertext_and_tag: z
        .string()
        .regex(/^[0-9a-fA-F]*$/, 'Must be valid hex string'),
    }),
  })
  .transform((data) => {
    return {
...
export type EncryptedPayloadV1 = z.infer<typeof EncryptedPayloadV1Schema>;
