...
async function hashDirectory(
  ...
  for (const file of files) {
    if (file.endsWith('.ts') && !file.includes('spec')) {
      // Exclude test files like this one
...
describe('ACC interfaces', () => {
  ...
  it('should have updated TS definitions based on JSON schemas', async () => {
    const interfacesDirectory =
      '../../../../types/src/generated/access-control-conditions';

    const initialHashes = await hashDirectory(
      path.join(__dirname, interfacesDirectory)
    );

    // update TS interfaces based on current JSON schemas
    await runNpmScript('yarn update:encryption --gen');

    const updatedHashes = await hashDirectory(
      path.join(__dirname, interfacesDirectory)
    );
