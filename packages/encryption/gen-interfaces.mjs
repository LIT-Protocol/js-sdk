...
const files = (await getFiles(INPUT_DIR)).filter((file) =>
  file.includes('.json')
);

const jsonFiles = await Promise.all(
  files.map((file) => readJsonFile(`${INPUT_DIR}/${file}`))
);
...
const indexFileContent = jsonFiles
  .map((jsonFile) => `export * from './${jsonFile.title}';`)
  .join('\n');
