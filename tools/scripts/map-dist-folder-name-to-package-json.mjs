import nxDevkit from '@nrwl/devkit';
...
const readCachedProjectGraph = nxDevkit.readCachedProjectGraph;
    ...
    Object.entries(nodes).forEach(async (node, i) => {
      if (node[1].data.projectType === 'library') {
        const packageJsonPath =
          'dist/packages/' + node[1].name + '/' + 'package.json';
        ...
        try {
          ...
          let newPath =
            'dist/packages/' + node[1].name + '-vanilla/package.json';
