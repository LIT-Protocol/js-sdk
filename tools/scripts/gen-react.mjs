...
const banner = '(REACT) THIS FILE IS AUTOMATICALLY GENERATED FROM tools/scripts/gen-react.mjs';
...
const modules = (await listDirsRecursive(DIST_DIR, false))
    ...
    .map((dir) => {
        ...
        const moduleName = (dir.split('/').pop()).replace(/-([a-z])/g, (g) => g[1].toUpperCase());
        return {
            moduleName,
            dir
        };
...
const importTags = modules.map((mod) => `import * as ${globalVarPrefix
    }${mod.moduleName} from '${mod.dir}';`);
...
const declareGlobal = getDeclareGlobalTemplate(
    modules.map((mod) => `${globalVarPrefix}${mod.moduleName}: any;`)
        .join('\n    ')
);

const getBodyTemplate = (_useEffectBody = '// FILL THIS UP!', _returnBody = '// RETURN BODY') => {
...
const consoleLogs = modules.map((mod, i) => {
    return getConsoleTemplate(globalVarPrefix + mod.moduleName, i, globalVarPrefix, true);
}).join('\n');
