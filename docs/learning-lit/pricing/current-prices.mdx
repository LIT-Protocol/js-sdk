...
- **Documentation prices are per request**: The prices shown in this documentation are multiplied by the threshold number of nodes, since your request will be sent to the threshold nodes and each node charges the product price. This gives you the total cost you'll pay per request.
- **Threshold calculation**: The threshold is calculated as `floor(total_nodes × 2/3)` using integer math (truncation), with a minimum of 3 nodes. For example:
...
- **Total cost calculation**: When you make a request, the total cost is `product_price × threshold_nodes`, because each threshold node charges you the product price.

For example, if the on-chain price for PKP Sign is 0.01 LITKEY per node, and there are 8 total nodes (5 threshold nodes), the total cost per request shown here will be 0.05 LITKEY (0.01 × 5).
</Warning>
