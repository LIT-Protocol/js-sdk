...
**Pricing Note:** The costs shown here are per request (already multiplied by the threshold number of nodes). On-chain prices are per node, but since your request goes to threshold nodes (calculated as `floor(total_nodes × 2/3)` with a minimum of 3) and each node charges the product price, the total cost per request is `product_price × threshold_nodes`.
</Note>
