...
- **Total cost per request**: When you make a request, it is sent to multiple nodes (typically N nodes based on the network's threshold requirements). Each node charges you the product price, so your total cost per request is `product_price × number_of_nodes`.
- **Documentation prices**: The prices shown in our documentation pages are already multiplied by the number of nodes to show you the total cost per request, making it easier to understand what you'll actually pay.
</Note>
