...
By default, all tokens that are staked remain locked for the timelock that is selected until the unlock is triggered. At that time, stake weight will continue to decay until the tokens are fully unlocked. 
