 ...
 */
export const runInBand = async ({ tests, devEnv, }) => {
    const filters = getFiltersFlag();
    const testsToRun = Object.entries(tests).filter(([testName]) => filters.length === 0 || filters.includes(testName));
        ...
        while (attempts < maxAttempts && !testPassed) {
            ...
            try {
                ...
                if (attempts >= maxAttempts) {
                    const endTime = performance.now();
                    const timeTaken = (endTime - startTime).toFixed(2);
                    console.error(`${testName} - Failed after ${maxAttempts} attempts (${timeTaken} ms)`);
                    console.error(`Error: ${error}`);
                    failedTests.push(`${testName} (Failed in ${timeTaken} ms) - Error: ${error}`);
                }
            }
            await new Promise((resolve) => setTimeout(resolve, devEnv.processEnvs.RUN_IN_BAND_INTERVAL));
        }
    }
    ...
    console.log();
    // Print results
    console.log(`Test Report: ${passedTests.length} test(s) passed, ${failedTests.length} failed, ${skippedTests.length} skipped.`);
 ...
 */
export const runTestsParallel = async ({ tests, devEnv, chunkSize, }) => {
    ...
    const testsToRun = Object.entries(tests).filter(([testName]) => (filters.length === 0 ||
    ...
    if (!testsToRun || testsToRun.length <= 0) {
        throw new Error('❌ No tests to run. You might have provided an invalid filter or no tests are available.');
    }
    const runTest = async ([testName, testFunction], testIndex) => {
        const maxAttempts = devEnv.processEnvs.MAX_ATTEMPTS;
        ...
        while (attempts < maxAttempts && !testPassed) {
            const startTime = performance.now();
            try {
                console.log(`\x1b[90m[runTestsParallel] Attempt ${attempts + 1} for ${testIndex + 1}. ${testName}...\x1b[0m`);
                await withTimeout(testFunction(devEnv), testTimeout);
                testPassed = true;
                const endTime = performance.now();
                const timeTaken = (endTime - startTime).toFixed(2);
                console.log(`\x1b[32m✔\x1b[90m ${testIndex + 1}. ${testName} - Passed (${timeTaken} ms)\x1b[0m`);
                ...
                if (error.message === 'Timed out') {
                    console.error(`\x1b[31m✖\x1b[90m ${testIndex + 1}. ${testName} - Timed out after ${testTimeout}ms (${timeTaken} ms)\x1b[0m`);
                    return `${testName} (Timed out in ${timeTaken} ms)`;
                }
                if (attempts >= maxAttempts) {
                    console.error(`\x1b[31m✖\x1b[90m ${testIndex + 1}. ${testName} - Failed after ${maxAttempts} attempts (${timeTaken} ms)\x1b[0m`);
                    console.error(`\x1b[31m❌Error:\x1b[90m ${JSON.stringify(error) || error}\x1b[0m`);
    ...
    for (let i = 0; i < testsToRun.length; i += CHUNK_SIZE) {
        ...
        const chunkResults = await Promise.all(chunk.map((test, index) => runTest(test, i + index)));
    ...
    if (skippedTests.length > 0) {
        ...
        skippedTests.forEach((skippedTest) => console.log(`\x1b[90m- ${skippedTest}\x1b[0m`));
    }
    if (failedTests.length > 0) {
        console.log(`\x1b[31mTest Report: Some tests failed.\x1b[0m`);
        failedTests.forEach((failedTest) => console.log(`\x1b[31m- ${failedTest}\x1b[0m`));
    }
    if (passedTests.length > 0) {
        console.log(`\x1b[32mTest Report: ${passedTests.length} test(s) passed.\x1b[0m`);
        passedTests.forEach((passedTest) => console.log(`\x1b[32m- ${passedTest}\x1b[0m`));
    }
    if (failedTests.length > 0) {
        console.log(`\x1b[31mTest Report: ${failedTests.length} test(s) failed.\x1b[0m`);
        return 1; // Exit with error code if any test failed
    }
    else {
...
//# sourceMappingURL=tinny-operations.js.map