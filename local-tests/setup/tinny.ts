...
import { craftAuthSig, createSiweMessage } from '@lit-protocol/auth-helpers';
...
export class Person {
  ...
  async spawn() {
    log('🧪 [tinny.ts] Spawning person:', this.wallet.address);
    /**
     * ====================================
     ...
     */
    log(
      '🧪 [tinny.ts] Crafting an authMethod from the authSig for the eth wallet auth method...'
    );
     ...
     * ====================================
     */
    log('🧪 [tinny.ts] Minting a PKP with eth wallet auth method...');
  ...
  async createCapacityDelegationAuthSig(
    ...
    log(
      '🧪 [tinny.ts] Mint a Capacity Credits NFT and get a capacity delegation authSig with it'
    );
    const capacityTokenId = (
      await this.contractsClient.mintCapacityCreditsNFT({
        requestsPerKilosecond:
...
export class TinnyEnvironment {
  ...
  constructor(network?: LIT_TESTNET) {
    ...
    // start a for loop that print out all the available private keys every 2 seconds
    setInterval(() => {
      this.processEnvs.KEY_IN_USE.forEach((used, index) => {
        log(
          `🔑 Private key at index ${index} is ${used ? 'in use' : 'available'}`
        );
      });
    }, 2000);
  }
  ...
  async getAvailablePrivateKey(): Promise<{
    ...
    while (true) {
      ...
      if (index !== -1) {
        ...
        log('🔑 Selected key at index', index); // Log a message indicating that we have selected a key

        // Set a timer to automatically release the key after 10 seconds
        setTimeout(() => {
          this.releasePrivateKey(index);
          log(
            '🔓 Automatically released key at index',
            index,
            `after ${this.processEnvs.TIME_TO_RELEASE_KEY / 1000} seconds`
          );
        ...
        log('No available keys. Waiting...'); // Log a message indicating that we are waiting
  ...
  releasePrivateKey(index: number) {
    ...
    log(`🪽 Released key at index ${index}. Thank you for your service!`);
  }
  ...
  async createNewPerson(name: string) {
    log('🧪 [tinny.ts] Creating new person:', name);
    if (!name) {
    ...
    log(
      '🧪 [tinny.ts] Creating new person with private key:',
      JSON.stringify(key)
    );
    log('🧪 [tinny.ts] Creating new person with envConfig:', envConfig);
  ...
  async getOrCreatePerson(name: string) {
    return this.getPerson(name) || (await this.createNewPerson(name));
  }

  async createRandomPerson() {
    // return await this.getOrCreatePerson('alice');
    // const randomName = this.litNodeClient.getRandomHexString(12);
  ...
  setPkpSignVersion = (network: LIT_TESTNET, version: LIT_ENDPOINT_VERSION) => {
    if (this.processEnvs.NETWORK === network) {
