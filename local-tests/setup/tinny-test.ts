...
export const runTests = async ({
  ...
  for (const [testName, testFunction] of testsToRun) {
    ...
    try {
      ...
      // Log success with a tick and grey color
      console.log(
      ...
      if (error.message !== 'Unavailable') {
        ...
        // Log failure with a cross and grey color
        console.error(
...
export const runTestsParallel = async ({
  ...
  const testPromises = testsToRun.map(
    async ([testName, testFunction], index) => {
      ...
      try {
        console.log(
          `\x1b[90m[runTestsParallel] Running ${
            index + 1
          }. ${testName}...\x1b[0m`
        );

        // @ts-ignore
        await testFunction(devEnv); // Execute test function
        ...
        console.error(
          `\x1b[31m✖\x1b[90m ${
            index + 1
          }. ${testName} - Failed (${timeTaken} ms)\x1b[0m`
        );
        console.error(`\x1b[31mError:\x1b[90m ${error.message}\x1b[0m`);
