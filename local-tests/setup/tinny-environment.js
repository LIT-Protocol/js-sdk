...
import { ShivaClient } from './shiva-client';
import { toErrorWithMessage } from './tinny-utils';
import { CENTRALISATION_BY_NETWORK, LIT_NETWORK, RPC_URL_BY_NETWORK, } from '@lit-protocol/constants';
...
export class TinnyEnvironment {
    constructor(network) {
        ...
        this.processEnvs = {
            ...
            NETWORK: process.env['NETWORK'] || LIT_NETWORK.Custom,
            DEBUG: process.env['DEBUG'] === 'true',
            REQUEST_PER_KILOSECOND: parseInt(process.env['REQUEST_PER_KILOSECOND']) ||
                process.env['NETWORK'] === 'datil-dev'
                ? 1
                : 200,
            LIT_RPC_URL: process.env['LIT_RPC_URL'],
            WAIT_FOR_KEY_INTERVAL: parseInt(process.env['WAIT_FOR_KEY_INTERVAL']) || 3000,
        ...
        this.bareSolAuthSig = {
            ...
            signedMessage: 'I am creating an account to use Lit Protocol at 2024-05-08T16:39:44.481Z',
            address: 'F7r6ENi6dqH8SnMYZdK3YxWAQ4cwfSNXZyMzbea5fbS1',
        };
        this.bareCosmosAuthSig = {
            sig: 'dE7J8oaWa8zECuMpaI/IVfJXGpLAO1paGLho+/dmtaQkN7Sh1lmJLAdYqZchDyYhQcg+nqfaoEOzLig3CPlosg==',
            derivedVia: 'cosmos.signArbitrary',
            signedMessage: '8c857343720203e3f52606409e6818284186a614e74026998f89e7417eed4d4b',
            address: 'cosmos14wp2s5kv07lt220rzfae57k73yv9z2azrmulku',
        };
        //=========== PRIVATE MEMBERS ===========
        this._shivaClient = new ShivaClient();
        this.world = new Map();
        this.setUnavailable = (network) => {
         ...
         */
        this.getFunds = async (walletAddress, amount = '0.001') => {
         ...
         * max capacity.
         */
        this.setupSuperCapacityDelegationAuthSig = async () => {
            const privateKey = await this.getAvailablePrivateKey();
            const provider = new ethers.providers.JsonRpcBatchProvider(this.rpc);
            const wallet = new ethers.Wallet(privateKey.privateKey, provider);
            /**
            ...
            if (this.network === LIT_NETWORK.Custom) {
                const networkContext = this?.testnet?.ContractContext ?? this._contractContext;
        ...
        if (Object.values(LIT_NETWORK).indexOf(this.network) === -1) {
            throw new Error(`Invalid network environment. Please use one of ${Object.values(LIT_NETWORK)}`);
        }
        // -- create an empty array to keep track of all keys
        this.processEnvs.KEY_IN_USE = new Array(this.processEnvs.PRIVATE_KEYS.length).fill(false);
        // -- setup rpc
        // Priority:
        ...
        if (this.processEnvs.LIT_RPC_URL) {
            ...
            throw new Error(`No RPC URL found for network "${this.network}". Available networks are: ${availableNetworks}`);
        }
        console.log('[𐬺🧪 Tinny Environment𐬺] Done configuring environment current config: ', this.processEnvs);
    }
    /**
    ...
    async getAvailablePrivateKey() {
        while (true) {
            ...
            if (index !== -1) {
                ...
                await new Promise((resolve) => setTimeout(resolve, this.processEnvs.WAIT_FOR_KEY_INTERVAL));
            }
        }
    }
    /**
     ...
     */
    releasePrivateKeyFromUser(user) {
    ...
    async setupLitNodeClient() {
        ...
        if (globalThis.wasmExports) {
            console.warn('WASM modules already loaded. Will override when connect is called');
        }
        if (globalThis.wasmECDSA) {
            console.warn('WASM modules already loaded. wil override. when connect is called');
        }
     ...
     */
    getEnvConfig() {
        const contractContext = this?.testnet?.ContractContext ?? this._contractContext;
        return {
            rpc: this.rpc,
    ...
    async init() {
        try {
            ...
            if (this.network === LIT_NETWORK.Custom && this.processEnvs.USE_SHIVA) {
                ...
                if (state === `UNKNOWN`) {
                    console.log('Testnet state found to be Unknown meaning there was an error with testnet creation. shutting down');
            ...
            console.log(`[𐬺🧪 Tinny Environment𐬺] Failed to init() tinny ${err.message}`);
    ...
    async mintSuperCapacityDelegationAuthSig(wallet) {
        console.log('[𐬺🧪 Tinny Environment𐬺] Mint a Capacity Credits NFT and get a capacity delegation authSig with it');
        ...
        try {
            ...
            if (e.message.includes(`Can't allocate capacity beyond the global max`)) {
                ...
                console.log('❗️Error while setting up capacity delegation auth sig', e);
            }
        }
    }
}
//# sourceMappingURL=tinny-environment.js.map