 ...
 */
export const testPkpEthersWithLitActionSessionSigsToEthSignTransaction = async (devEnv) => {
    ...
    try {
        ...
        const txHash = ethers.utils.keccak256(ethers.utils.serializeTransaction(rawTx));
        ...
        if (signature.length !== 132) {
            throw new Error(`❌ signature should be 132 characters long, got ${signature.length}`);
        }
        if (recoveredAddress.toLowerCase() !== alice.pkp.ethAddress.toLowerCase()) {
            throw new Error(`❌ recoveredAddres should be ${alice.pkp.ethAddress}, got ${recoveredAddress}`);
        }
    }
    catch (e) {
        if (e.message.includes('insufficient FPE funds')) {
            console.log(`🧪 PKPEthersWallet should be able to send tx (insufficient FPE funds ❗️)`);
        }
        else {
...
//# sourceMappingURL=testPkpEthersWithLitActionSessionSigsToEthSignTransaction.js.map