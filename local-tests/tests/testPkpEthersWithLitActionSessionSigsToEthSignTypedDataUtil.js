import { PKPEthersWallet, signTypedData } from '@lit-protocol/pkp-ethers';
 ...
 */
export const testPkpEthersWithLitActionSessionSigsToEthSignTypedDataUtil = async (devEnv) => {
    ...
    try {
        ...
        const recoveredAddr = ethers.utils.verifyTypedData(msgParams.domain, { Person: msgParams.types.Person, Mail: msgParams.types.Mail }, msgParams.message, signature);
        if (signature.length !== 132) {
            throw new Error('❌ signature should be 132 characters long');
        }
        if (recoveredAddr.toLowerCase() !== alice.pkp.ethAddress.toLowerCase()) {
            throw new Error(`❌ recoveredAddr ${recoveredAddr} should be ${alice.pkp.ethAddress}`);
        }
    }
    catch (e) {
...
//# sourceMappingURL=testPkpEthersWithLitActionSessionSigsToEthSignTypedDataUtil.js.map