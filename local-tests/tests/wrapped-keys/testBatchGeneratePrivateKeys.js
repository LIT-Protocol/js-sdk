...
async function verifySolanaSignature(solanaResult, solanaMessageToSign) {
    const { signMessage: { signature }, generateEncryptedPrivateKey: { generatedPublicKey }, } = solanaResult;
    const signatureIsValidForPublicKey = nacl.sign.detached.verify(Buffer.from(solanaMessageToSign), bs58.decode(signature), bs58.decode(generatedPublicKey));
    console.log({ signatureIsValidForPublicKey, signature });
    if (!signatureIsValidForPublicKey) {
        throw new Error(`signature: ${signature} doesn't validate for the Solana public key: ${generatedPublicKey}`);
    }
}
async function verifyEvmSignature(evmResult, messageToSign) {
    function verifyMessageSignature() {
        try {
            return ethers.utils.verifyMessage(messageToSign, evmResult.signMessage.signature);
        }
        catch (err) {
            throw new Error(`When validating signed Ethereum message is valid: ${err.message}`);
        }
    }
    const walletAddress = ethers.utils.computeAddress(evmResult.generateEncryptedPrivateKey.generatedPublicKey);
    ...
    if (recoveredAddress !== walletAddress) {
        throw new Error("Recovered address from verifyMessage doesn't match the wallet address");
    }
}
/**
 * Test Commands:
 ...
 */
export const testBatchGeneratePrivateKeys = async (devEnv) => {
    ...
    try {
        ...
        if (results.length !== 2) {
            throw new Error(`Incorrect # of results; expected 2, got ${results.length}`);
        }
        if (results[0].generateEncryptedPrivateKey.memo !== 'Test evm key' ||
            results[1].generateEncryptedPrivateKey.memo !== 'Test solana key') {
            throw new Error('Results not in order sent; expected evm as first result, solana as second');
        }
        if (!results[0].signMessage.signature ||
...
//# sourceMappingURL=testBatchGeneratePrivateKeys.js.map