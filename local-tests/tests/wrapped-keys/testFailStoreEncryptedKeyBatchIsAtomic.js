...
import { getFirstSessionSig, getKeyTypeFromNetwork, getPkpAccessControlCondition, getPkpAddressFromSessionSig, } from '../../../packages/wrapped-keys/src/lib/api/utils';
 ...
 */
export const testFailBatchGeneratePrivateKeysAtomic = async (devEnv) => {
    ...
    try {
        ...
        const { litActionCode, litActionIpfsCid } = getLitActionCodeOrCidCommon('batchGenerateEncryptedKeys');
        ...
        try {
            ...
            throw new Error('storeEncryptedKeyBatch() succeeded but we expected it to fail!');
        }
        catch (err) {
            // We expect `storeEncryptedKeyBatch` to fail w/ a specific error
            if (err.message.includes('storeEncryptedKeyBatch() succeeded but we expected it to fail!') ||
                !err.message.includes('keyParamsBatch[0]: Missing "publicKey" parameter in request')) {
                throw err;
            }
            try {
                ...
                console.error('Got a value back we shouldnt have from listEncryptedKeyMetadata()', keys);
                throw new Error('Expected `listEncryptedKeyMetadata() to fail, but it didnt!`');
            }
            catch (err) {
...
//# sourceMappingURL=testFailStoreEncryptedKeyBatchIsAtomic.js.map