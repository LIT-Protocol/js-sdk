 ...
 */
export const testFailEthereumSignTransactionWrappedKeyInvalidDecryption = async (devEnv) => {
    const alice = await devEnv.createRandomPerson();
    const bob = await devEnv.createRandomPerson();
    try {
        const privateKey = ethers.Wallet.createRandom().privateKey;
        const alicePkpAddress = alice.authMethodOwnedPkp.ethAddress;
        const decryptionAccessControlCondition = getPkpAccessControlCondition(alicePkpAddress);
        ...
        const pkpSessionSigsSigning = await getPkpSessionSigs(devEnv, bob, null, new Date(Date.now() + 1000 * 60 * 10).toISOString()); // 10 mins expiry
        ...
        try {
            ...
            if (e.message.includes('There was an error getting the signing shares from the nodes')) {
                console.log('✅ THIS IS EXPECTED: ', e);
                console.log(e.message);
                console.log('✅ testFailEthereumSignTransactionWrappedKeyInvalidDecryption is expected to have an error');
            }
            else {
                throw e;
            }
...
//# sourceMappingURL=testFailEthereumSignTransactionWrappedKeyInvalidDecryption.js.map