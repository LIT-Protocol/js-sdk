 ...
 */
export const testSignMessageWithSolanaEncryptedKey = async (devEnv) => {
    ...
    try {
        ...
        console.log(signature);
        const signatureIsValidForPublicKey = nacl.sign.detached.verify(Buffer.from(messageToSign), ethers.utils.base58.decode(signature), solanaKeypair.publicKey.toBuffer());
        if (!signatureIsValidForPublicKey)
            throw new Error(`signature: ${signature} doesn't validate for the Solana public key: ${solanaKeypair.publicKey.toString()}`);
...
//# sourceMappingURL=testSignMessageWithSolanaEncryptedKey.js.map