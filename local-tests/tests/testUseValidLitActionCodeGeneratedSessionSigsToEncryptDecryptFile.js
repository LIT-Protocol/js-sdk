 ...
 * ✅ NETWORK=datil-dev yarn test:local --filter=testUseValidLitActionCodeGeneratedSessionSigsToEncryptDecryptFile
 */
export const testUseValidLitActionCodeGeneratedSessionSigsToEncryptDecryptFile = async (devEnv) => {
    ...
    const accsResourceString = await LitAccessControlConditionResource.generateResourceString(accs, encryptRes.dataToEncryptHash);
    ...
    if (blobArray.length !== decriptedFile.length) {
        throw new Error(`decrypted file should match the original file but received ${decriptedFile}`);
    }
...
//# sourceMappingURL=testUseValidLitActionCodeGeneratedSessionSigsToEncryptDecryptFile.js.map