...
const PACKAGES = [
  'auth-services', 'networks', 'auth', 'lit-client', 'auth-helpers',
  'crypto', 'types', 'logger', 'schemas', 'constants', 'wasm',
  ...
  for (const pkg of PACKAGES) {
    ...
    try {
      ...
      try {
        ...
        if (error.stdout) {
          try {
            ...
            console.log(`  ❌ Failed to parse depcheck output for ${pkg}: ${parseError.message}\n`);
            continue;
          }
        } else {
          console.log(`  ❌ Failed to run depcheck for ${pkg}: ${error.message}\n`);
          continue;
        }
      }
      ...
      if (realMissingDeps.length > 0) {
        console.log(`  ➕ Adding ${realMissingDeps.length} missing dependencies`);
        ...
        const sortedDeps = {};
        Object.keys(pkgJson.dependencies).sort().forEach(key => {
      ...
      console.log(`  ❌ Unexpected error processing ${pkg}: ${error.message}\n`);
    }
  }
