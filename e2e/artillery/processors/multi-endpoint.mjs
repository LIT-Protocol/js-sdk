...
} from '../../src/helper/tests/index';

// Shared context - singleton pattern following the e2e tests
let sharedContext = null;
let alicePkpAuthContext = null;
let aliceCustomAuthContext = null;
 ...
 */
async function runTestWithMetrics(testName, testFunction, context, events, parallelism = 1) {
  const testPromises = Array.from({ length: parallelism }).map(async (_, index) => {
    ...
    try {
      ...
      console.error(`❌ ${testName} failed (attempt ${index + 1}):`, error.message);
  ...
  console.log(`📊 ${testName}: ${successCount}/${parallelism} success, avg: ${avgDuration.toFixed(2)}ms`);
 ...
 */
export async function runMultiEndpointTest(context, events) {
...
export async function runPkpSignTestWithEoa(context, events) {
  ...
  await runTestWithMetrics('pkp_sign_eoa', testFn, context, events, parallelism);
}
...
export async function runPkpSignTestWithPkp(context, events) {
  ...
  await runTestWithMetrics('pkp_sign_pkp', testFn, context, events, parallelism);
}
...
export async function runPkpSignTestWithCustom(context, events) {
  ...
  await runTestWithMetrics('pkp_sign_custom', testFn, context, events, parallelism);
}
...
export async function runEncryptDecryptTest(context, events) {
  ...
  await runTestWithMetrics('encrypt_decrypt', testFn, context, events, parallelism);
}
...
export async function runPkpEncryptDecryptTest(context, events) {
  ...
  await runTestWithMetrics('pkp_encrypt_decrypt', testFn, context, events, parallelism);
}
...
export async function runEncryptDecryptFlowTest(context, events) {
  ...
  await runTestWithMetrics('encrypt_decrypt_flow', testFn, context, events, parallelism);
}
...
export async function runViewPkpsTest(context, events) {
  ...
  const viewByAuthData = createViewPKPsByAuthDataTest(sharedContext, authContext);
  
  await Promise.all([
    runTestWithMetrics('view_pkps_by_address', viewByAddress, context, events, Math.ceil(parallelism / 2)),
    runTestWithMetrics('view_pkps_by_auth_data', viewByAuthData, context, events, Math.floor(parallelism / 2))
  ]);
}
...
export async function runViemSignTest(context, events) {
  ...
  const signTransaction = createViemSignTransactionTest(sharedContext, authContext);
  const signTypedData = createViemSignTypedDataTest(sharedContext, authContext);
  
  await Promise.all([
    runTestWithMetrics('viem_sign_message', signMessage, context, events, Math.ceil(parallelism / 3)),
    runTestWithMetrics('viem_sign_transaction', signTransaction, context, events, Math.ceil(parallelism / 3)),
    runTestWithMetrics('viem_sign_typed_data', signTypedData, context, events, Math.floor(parallelism / 3))
  ]);
} 