...
async function main() {
  try {
    ...
    console.log(`\n✅ All tests completed!`);
    console.log(`📈 Results: ${allTestsResults.passed}/${allTestsResults.totalTests} passed`);
    ...
    console.log(`📈 Results: ${signingResults.passed}/${signingResults.totalTests} passed`);
    ...
    if (detailedResults.failed > 0) {
      ...
      const failedTests = detailedResults.results.filter(r => r.status === 'failed');
    ...
    console.log(`  EOA Auth: ${detailedResults.summary.eoaAuth.passed} passed, ${detailedResults.summary.eoaAuth.failed} failed`);
    console.log(`  PKP Auth: ${detailedResults.summary.pkpAuth.passed} passed, ${detailedResults.summary.pkpAuth.failed} failed`);
    console.log(`  Custom Auth: ${detailedResults.summary.customAuth.passed} passed, ${detailedResults.summary.customAuth.failed} failed`);
    console.log(`  EOA Native: ${detailedResults.summary.eoaNative.passed} passed, ${detailedResults.summary.eoaNative.failed} failed`);

    // Performance analysis
    console.log('\n⚡ Performance Analysis:');
    const avgDuration = detailedResults.results.reduce((sum, r) => sum + r.duration, 0) / detailedResults.results.length;
    console.log(`  Average test duration: ${avgDuration.toFixed(2)}ms`);
    
    const slowestTest = detailedResults.results.reduce((max, r) => r.duration > max.duration ? r : max);
