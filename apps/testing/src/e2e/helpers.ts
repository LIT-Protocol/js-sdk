...
export const helpers = {
  ...
  prepareMetamask: async version => {
    const release = await helpers.getMetamaskReleases(version);
    const downloadsDirectory = path.resolve(__dirname, 'downloads');
    await helpers.createDirIfNotExist(downloadsDirectory);
    const metamaskDirectory = path.join(downloadsDirectory, release.tagName);
    const metamaskDirectoryExists = await helpers.checkDirOrFileExist(
      metamaskDirectory,
    );
    ...
    const metamaskManifestFileExists = await helpers.checkDirOrFileExist(
      metamaskManifestFilePath,
    );
    if (!metamaskDirectoryExists && !metamaskManifestFileExists) {
      await helpers.download(release.downloadUrl, metamaskDirectory);
    }
