...
export const metamask = {
  ...
  acceptAccess: async allAccounts => {
    ...
    console.log("METAMASK:", allAccounts);
    const notificationPage = await playwright.switchToMetamaskNotification();
  ...
  initialSetup: async ({ secretWordsOrPrivateKey, network, password }) => {
    ...
    console.log("Location -> We are here!");
    ...
    if (
      ...
      await metamask.confirmWelcomePage();
      if (secretWordsOrPrivateKey.includes(' ')) {
        // secret words
        await metamask.importWallet(secretWordsOrPrivateKey, password);
      } else {
        // private key
        await metamask.createWallet(password);
        await metamask.importAccount(secretWordsOrPrivateKey);
      }
      if (isCustomNetwork) {
      ...
      walletAddress = await metamask.getWalletAddress();
