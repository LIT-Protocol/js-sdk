...
export const playwright = {
  ...
  switchToCypressWindow: async () => {
    ...
    await playwright.assignActiveTabName('cypress');
    return true;
  },
  ...
  switchToMetamaskNotificationWindow: async () => {
    ...
    await playwright.assignActiveTabName('metamask-notif');
    return true;
  },
  switchToMetamaskNotification: async () => {
    ...
    if (retries < 50) {
      retries++;
      return await playwright.switchToMetamaskNotification();
    } else if (retries >= 50) {
      retries = 0;
      throw new Error(
  ...
  waitAndClick: async (selector, page = metamaskWindow, args = {}) => {
    const element = await playwright.waitFor(selector, page);
  ...
  waitAndClickByText: async (selector, text, page = metamaskWindow) => {
    await playwright.waitFor(selector, page);
  ...
  waitClearAndType: async (text, selector, page = metamaskWindow) => {
    const element = await playwright.waitAndClick(selector, page, {
      numberOfClicks: 3,
    });
