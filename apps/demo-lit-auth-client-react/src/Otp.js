...
export function Otp() {
    ...
    const onEnterStart = async(e) => {
        ...
        let session = await authClient.signInWithOTP({
            userId: '+1' + transport,
        });
    ...
    const onEnterCheckRegister = async (e) => {
        ...
        const res = await authClient.mintPKPWithAuthMethod(authMethod);
        console.log(res);
        setPkpInfo(res);
    ...
    const onEnterCheckLogin = async (e) => {
        ...
        console.log(authMethod);
        setAccessToken(authMethod.accessToken);
        const res = await authClient.fetchPKPsByAuthMethod(authMethod);
        console.log(res);
    ...
    if (state === 'start') {
        ...
        return (
            <div>
                ...
                <br/>
                <div className='context-buttons'>
                    <button onClick={onSignSessionsig}>Login</button>
                </div>
            </div>
        );
